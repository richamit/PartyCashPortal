...
data = LOAD '/user/ubuntu/richa/random/filling_date/out/part-r-*' USING PigStorage('|') AS (type:chararray,party:chararray,dat:chararray,state:chararray, amount:double);
...
STORE data3 INTO '/user/ubuntu/richa/random/filling_date/out_final' USING PigStorage('|') ;

data4 = LOAD '/user/ubuntu/richa/random/filling_date/out_final/part-m-*' USING PigStorage('|') AS (htkey:chararray, amount:double);
