SET job.name 'richa_election_state_filling';
load_contribution = LOAD '/user/ubuntu/richa/random/filling_date/in/*'  USING PigStorage('|') AS (CMTE_ID:chararray,AMNDT_IND:chararray,RPT_TP:chararray,TRANSACTION_PGI:chararray,
...
STORE ready INTO '/user/ubuntu/richa/random/filling_date/out' USING PigStorage('|') ;